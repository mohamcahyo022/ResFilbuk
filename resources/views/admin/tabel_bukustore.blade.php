              ...
              <table id="daftar_postingan" class="table table-striped table-hover align-items-center mb-0">
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <p class="text-xs text-secondary mb-0">{{ $buku->genre }}</p>
                    </td>
                    <td class="align-middle text-center">
                      <span class="text-secondary text-xs font-weight-bold">{{ $buku->rilis }}</span>
                    </td>

                    <td>
                        <center>
                      <form action="/edit-perpus/{{$buku->id}}" method="post" class="d-inline" >
                        @method('put')
                        @csrf
                        <button type="submit" class="badge bg-gradient-success border-0">Edit</button>
                      </form>
                      <form action="/hapus-perpus/{{ $buku->id }}" method="post" class="d-inline" >
                        @method('delete')
                        @csrf
